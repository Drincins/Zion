...
def _kpi_metric_public_columns():
    return (
        KpiMetric.id,
        KpiMetric.code,
        KpiMetric.name,
        KpiMetric.description,
        KpiMetric.unit,
        KpiMetric.calculation_base,
        ...
        KpiMetric.restaurant_ids,
        KpiMetric.bonus_adjustment_type_id,
        KpiMetric.penalty_adjustment_type_id,
    )


def _kpi_metric_group_public_columns():
    return (
        KpiMetricGroup.id,
        KpiMetricGroup.name,
        KpiMetricGroup.description,
        KpiMetricGroup.unit,
        KpiMetricGroup.use_max_scale,
        KpiMetricGroup.max_scale_value,
        KpiMetricGroup.plan_direction,
        KpiMetricGroup.plan_target_percent,
        KpiMetricGroup.bonus_adjustment_type_id,
        KpiMetricGroup.penalty_adjustment_type_id,
        KpiMetricGroup.created_at,
        KpiMetricGroup.updated_at,
    )


def _metric_public_load_options():
    return (
        load_only(*_kpi_metric_public_columns()),
        joinedload(KpiMetric.group).load_only(*_kpi_metric_group_public_columns()),
    )


def _rule_public_load_options():
    return (
        joinedload(KpiRule.metric)
        .load_only(*_kpi_metric_public_columns())
        .joinedload(KpiMetric.group)
        .load_only(*_kpi_metric_group_public_columns()),
    )


def _result_public_load_options():
    return (
        joinedload(KpiResult.metric)
        .load_only(*_kpi_metric_public_columns())
        .joinedload(KpiMetric.group)
        .load_only(*_kpi_metric_group_public_columns()),
    )
...
def _plan_fact_public_load_options():
    return (
        joinedload(KpiPlanFact.metric)
        .load_only(*_kpi_metric_public_columns())
        .joinedload(KpiMetric.group)
        .load_only(*_kpi_metric_group_public_columns()),
    )
...
class _PayoutScope:
    position_ref: object
    payment_mode: Optional[str]
    rest_minutes_map: dict[int, int]
    rest_night_minutes_map: dict[int, int]
    rest_rate_map: dict[int, Optional[Decimal]]
    total_minutes_map: dict[int, int]
    attendance_user_ids: set[int]
    restaurant_user_ids: set[int]
    user_map: dict[int, object]
    hours_per_shift: Optional[Decimal]
    monthly_shift_norm: Optional[Decimal]
    night_enabled: bool
    night_percent: Decimal


def _load_payout_scope(
    *,
    db: Session,
    restaurant_id: int,
    position_id: int,
    period_start: date,
    period_end: date,
    scope_cache: Optional[dict[tuple[int, int, date, date], _PayoutScope]] = None,
) -> _PayoutScope:
    cache_key = (restaurant_id, position_id, period_start, period_end)
    ...
    candidate_user_ids = attendance_user_ids | restaurant_user_ids
    user_map: dict[int, object] = {}
    if candidate_user_ids:
        user_rows = (
            db.query(User.id, User.position_id, User.rate)
    ...
    scope = _PayoutScope(
        position_ref=SimpleNamespace(id=position.id, rate=position.rate),
        payment_mode=payment_mode,
        rest_minutes_map=rest_minutes_map,
        rest_night_minutes_map=rest_night_minutes_map,
        rest_rate_map=rest_rate_map,
        total_minutes_map=total_minutes_map,
        attendance_user_ids=attendance_user_ids,
        restaurant_user_ids=restaurant_user_ids,
        user_map=user_map,
        hours_per_shift=_maybe_decimal(getattr(position, "hours_per_shift", None)),
        monthly_shift_norm=_maybe_decimal(getattr(position, "monthly_shift_norm", None)),
...
def _build_payout_batch(
    *,
    ...
    position = scope.position_ref
    payment_mode = scope.payment_mode

    if rule.employee_id:
        candidate_user_ids = {rule.employee_id}
    else:
        candidate_user_ids = set(scope.attendance_user_ids) | set(scope.restaurant_user_ids)
    ...
    missing_user_ids = set(candidate_user_ids) - set(scope.user_map)
    if missing_user_ids:
        user_rows = (
            db.query(User.id, User.position_id, User.rate)
        ...
        for row in user_rows:
            scope.user_map[int(row.id)] = SimpleNamespace(
                id=int(row.id),
                position_id=int(row.position_id) if row.position_id is not None else None,
                rate=row.rate,
            )
    user_map = scope.user_map
    ...
    hours_per_shift = scope.hours_per_shift
    monthly_shift_norm = scope.monthly_shift_norm
    night_enabled = scope.night_enabled
    night_percent = scope.night_percent
    ...
    for user_id in candidate_user_ids:
        ...
        rest_minutes = scope.rest_minutes_map.get(user_id, 0)
        rest_night_minutes = scope.rest_night_minutes_map.get(user_id, 0)
        total_minutes = scope.total_minutes_map.get(user_id, 0)
        ...
        stats_rate = scope.rest_rate_map.get(user_id)
...
def list_metrics(
    *,
    ...
    def _load_metrics() -> dict:
        query = db.query(KpiMetric).options(*_metric_public_load_options())
...
def list_plan_facts(
    *,
    ...
    items = (
        ...
        .options(*_plan_fact_public_load_options())
        .filter(
...
def upsert_plan_fact(
    ...
    entry = (
        db.query(KpiPlanFact)
        .options(*_plan_fact_public_load_options())
        .filter(KpiPlanFact.id == entry.id)
        .first()
    )
    if not entry:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Plan/fact not found after save")
...
def list_rules(
    *,
    ...
    query = (
        db.query(KpiRule)
        .options(*_rule_public_load_options())
...
def create_rule(
    ...
    rule = (
        db.query(KpiRule)
        .options(*_rule_public_load_options())
        .filter(KpiRule.id == rule.id)
        .first()
    )
    if not rule:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Rule not found after save")
...
def list_results(
    *,
    ...
    query = (
        db.query(KpiResult)
        .options(*_result_public_load_options())
...
def create_result(
    ...
    result = (
        db.query(KpiResult)
        .options(*_result_public_load_options())
        .filter(KpiResult.id == result.id)
        .first()
    )
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Result not found after save")
...
def create_payout_preview_by_metric(
    ...
    scope_cache: dict[tuple[int, int, date, date], _PayoutScope] = {}
