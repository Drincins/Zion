<template>
    <div class="kpi-page">
        ...
        <FiltersPanel v-model="isHistoryFiltersOpen" title="Фильтры">
            <div class="filters-panel__controls filters-panel__controls--end kpi-history-filters__controls">
                <Select
        ...
        <div v-if="showPreviewModal" class="kpi-modal">
            <div class="kpi-modal__dialog kpi-modal__dialog--preview">
                ...
                <section class="kpi-preview-metrics">
                    <div class="kpi-preview-metrics__header">
                        ...
                        <div class="kpi-preview-metrics__actions">
                            <Button color="ghost" size="sm" :disabled="previewLoading" @click="selectAllPreviewMetrics">
                                Выбрать все
                            </Button>
                            <Button color="ghost" size="sm" :disabled="previewLoading" @click="clearPreviewMetrics">
                                Очистить
                            </Button>
                        </div>
                    </div>
                ...
                <div class="kpi-panel__actions">
                    <Button
                        color="primary"
                        :disabled="!canManage || !selectedPreviewMetricIds.length || !selectedPreviewRestaurantIds.length || !previewForm.month"
        ...
        <div v-if="showPayoutModal" class="kpi-modal">
            <div class="kpi-modal__dialog kpi-modal__dialog--wide">
                <div class="kpi-modal__header">
                    <div>
                        ...
                        <p class="kpi-modal__subtitle">
                            ...
                            · Период: {{ formatDateRange(payoutModalPeriodStart, payoutModalPeriodEnd) }}
                        ...
                        <div v-if="payoutModalRows.length" class="kpi-summary">
                            ...
                            <div class="kpi-summary__grid">
                                <div class="kpi-summary__card">
                                    ...
                                    <p class="kpi-summary__meta">Сотрудников: {{ payoutSummary.row_count }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="kpi-modal__toolbar">
                    ...
                    <div class="kpi-modal__toolbar-right">
                        <Button
                            color="danger"
                            size="sm"
                            :disabled="!canManage || !payoutModalHasDrafts || postingBatch || deletingDrafts"
                ...
                <div class="kpi-modal__body">
                    <div class="kpi-posting">
                        ...
                        <div class="kpi-modal__field">
                            ...
                            <DateInput v-model="postForm.date" :disabled="!payoutModalHasDrafts || postingBatch || deletingDrafts" />
                        </div>
                        <Input
                    ...
                    <div class="kpi-table-wrapper">
                        <div v-if="payoutCalcSummary" class="kpi-payout-calc">
                            <span v-if="payoutCalcSummary.comparison_basis === 'plan_percent'">
                                План: <strong>{{ formatKpiValue(payoutCalcSummary.plan_value) }}</strong>
                                · Факт: <strong>{{ formatKpiValue(payoutCalcSummary.fact_value) }}</strong>
                                · Выполнение: <strong>{{ formatPercent(payoutCalcSummary.comparison_value) }}</strong>
                                · Порог: <strong>{{ formatPercent(payoutCalcSummary.target_value) }}</strong>
                            </span>
                            <span v-else>
                                Факт: <strong>{{ formatKpiValue(payoutCalcSummary.fact_value) }}</strong>
                                · Условие: <strong>{{ payoutCalcSummary.bonus_condition }} {{ formatKpiValue(payoutCalcSummary.target_value) }}</strong>
                            </span>
                        </div>
                                    ...
                                    <td>
                                        <div v-if="isRowEditing(row)" class="kpi-table__editor">
                                            <input
                                                ...
                                                type="number"
                                                step="0.01"
                                                :disabled="row.status !== 'draft' || postingBatch || deletingDrafts || savingItemId === row.item_id"
                                    ...
                                    <td class="kpi-table__actions">
                                        <div class="kpi-table__actions-inner">
                                            <button
                                                ...
                                                :disabled="row.status !== 'draft' || postingBatch || deletingDrafts || savingItemId === row.item_id || deletingItemId === row.item_id"
                                                :title="isRowEditing(row) ? 'Сохранить изменения' : 'Редактировать'"
                                            ...
                                            <button
                                                ...
                                                :disabled="row.status !== 'draft' || postingBatch || deletingDrafts || deletingItemId === row.item_id || savingItemId === row.item_id"
...
const selectedPreviewMetricIds = computed(() => {
    const source = Array.isArray(previewForm.value.metricIds) ? previewForm.value.metricIds : [];
    return Array.from(
        new Set(
            source
...
const selectedPreviewRestaurantIds = computed(() => {
    const source = Array.isArray(previewForm.value.restaurantIds) ? previewForm.value.restaurantIds : [];
    return Array.from(
        new Set(
            source
                .map((value) => Number(value))
...
const payoutModalRestaurantId = computed(() => payoutModalBatches.value?.[0]?.restaurant_id ?? null);
const payoutModalMetricId = computed(() => payoutModalBatches.value?.[0]?.metric_id ?? null);
const payoutModalPeriodStart = computed(() => payoutModalBatches.value?.[0]?.period_start ?? null);
...
const historyCards = computed(() => {
    ...
    for (const batch of batches.value || []) {
        ...
        if (batch.created_at && (!item.created_at || new Date(batch.created_at) > new Date(item.created_at))) {
            item.created_at = batch.created_at;
        }
        if (batch.posted_at && (!item.posted_at || new Date(batch.posted_at) > new Date(item.posted_at))) {
    ...
    return Array.from(groups.values())
        .map((item) => ({
            ...item,
            status: item.has_draft && item.has_posted ? 'partial' : (item.has_posted ? 'posted' : 'draft'),
...
const payoutCalcSummary = computed(() => {
    ...
    const snapshot = firstBatch?.calc_summary || (Array.isArray(firstBatch?.items) ? firstBatch.items?.[0]?.calc_snapshot : null);
            ...
            for (const metricId of metricIds) {
                ...
                try {
                    ...
                    failedPairs.push({
                        metricId,
                        restaurantId,
                        message: resolveApiErrorMessage(error, 'Не удалось сформировать выплату'),
                    });
                    console.error(error);
                }
            }
...
function hydratePostForm({ metricId, periodEnd, comment }) {
    ...
    postForm.value.adjustmentTypeId =
        ...
        (metric?.penalty_adjustment_type_id ? String(metric.penalty_adjustment_type_id) : null) ||
...
function openPayoutModal(batchesList, { metricId } = {}) {
    ...
    const resolvedMetricId = Number(first.metric_id || metricId || fallbackMetricId || 0) || null;
    hydratePostForm({
    ...
    try {
        ...
        for (const row of dirtyRows) {
            try {
                ...
                const reason = resolveApiErrorMessage(error, 'Не удалось сохранить строку перед записью');
        ...
        for (const batch of draftBatches) {
            const batchRowKeys = payoutModalRows.value
                .filter((row) => row.status === 'draft' && Number(row.batch_id) === Number(batch.id))
                .map((row) => row.key);
        ...
        if (updatedBatches.length) {
            const updatedMap = new Map(updatedBatches.map((batch) => [Number(batch.id), batch]));
            const nextBatches = payoutModalBatches.value.map((batch) => updatedMap.get(Number(batch.id)) || batch);
